# Nasnet model with dropout.
# Dropout 은 overfitting에 대한 방지 대책
...
for dirname, _, filenames in os.walk('/content/drive/My Drive/AI_Project/02_Projects/02_MinSeungJun'):
    for filename in filenames:
        os.path.join(dirname, filename)

import pandas as pd # 필요한 묘둘 import
import numpy as np
...
train_generator = train_data.flow_from_directory(directory=traindir,target_size=(331, 331),class_mode='binary',batch_size=32,shuffle=True)
...
vmodel_0 = Sequential()
vmodel_0.add(NasNet)
vmodel_0.add(layers.GlobalAveragePooling2D())
...
# 데이터 학습.
history_0 = vmodel_0.fit(train_generator,
...
# 학습된 모델 그래프로 plot하기
def plot_history(history, Option):
...
plot_history(history_0,'(LR = 0.01, Without Dropout)')

# LR = 0.01 에 대한 TEST 값
predictons_0 = vmodel_0.predict(test_generator)
...
vmodel_1 = Sequential()
vmodel_1.add(NasNet)
vmodel_1.add(layers.GlobalAveragePooling2D())
vmodel_1.add(layers.Dropout(0.4))
vmodel_1.add(layers.Dense(1, activation='sigmoid'))
vmodel_1.summary()
...
# 데이터 학습.
history_1 = vmodel_1.fit(train_generator,
                         steps_per_epoch=len(train_generator),
                         epochs=20,
                         validation_data=valid_generator,
                         batch_size = 128)

plot_history(history_1,'(LR = 0.01, With Dropout)')

# LR = 0.01 에 대한 TEST 값
predictons_1 = vmodel_1.predict(test_generator)
predictions_1 = np.round(predictons_1)

m = tf.keras.metrics.BinaryAccuracy()
y = np.hstack((np.zeros(50),np.ones(50)))
m.update_state(y, predictions_1)
m.result()
